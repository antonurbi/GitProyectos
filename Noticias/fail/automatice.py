...
def descargar_subtitulos(url_video):
    ...
    archivo_vtt = rf"{titulo_video}.es.vtt"
...
def descargar_subtitulos_utimosvideos(enlace,carpeta):
    ...
    if archivo_vtt:
        # Pasar solo el nombre del archivo a la función limpiar_subtitulos_con_saltos_de_linea
        texto_limpio = limpiar_subtitulos(archivo_vtt)
        archivo=f'\{titulo_video}_limpio.txt'
        path=carpeta + archivo
        
        with open(path, 'w', encoding='utf-8') as file_out: # ---> env\EconFinan\fecha\titulo_video_limpio.txt
